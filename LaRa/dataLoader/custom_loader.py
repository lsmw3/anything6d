...
class custom_loader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split=='train':
            src_view_id = random.sample(view_ids[4:-4], k=4)
            view_id = src_view_id + random.sample(view_ids[4:-4], k=4)
        else:
            src_view_id = list(view_ids[:4])
            view_id = src_view_id + list(view_ids[-4:])
    ...
    def get_input(self, instance, cam_params, view_id):
        tar_img, bg_colors, tar_nrms, tar_c2ws, tar_w2cs, tar_ixts = self.read_views(instance, cam_params, view_id)
    ...
    def read_views(self, instance, cam_params, src_views):
        ...
        for i, idx in enumerate(src_ids):
            ...
            img, normal = self.read_image(instance, idx, bg_color)
            imgs.append(img)
        ...
        return np.stack(imgs), np.stack(bg_colors), np.stack(normals), np.stack(exts), np.stack(w2cs), np.stack(ixts)
    ...
    def read_image(self, instance, view_idx, bg_color):
        img = np.array(Image.open(f"{instance}/image_{view_idx}.png"))[..., :3]
