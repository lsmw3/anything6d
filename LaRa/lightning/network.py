...
class VolTransformer(L.LightningModule):
    ...
    def forward(self, image_feats):
        ...
        for n_group in self.n_groups:
            ...
            if self.img_feats_avg:
                blocks = torch.mean(blocks, dim=1)
                blocks = torch.einsum('bcgl->bglc',blocks).reshape(B*n_group**3,block_size**3,C)
            else:
...
class Network(L.LightningModule):
    def __init__(self, cfg, white_bkgd=True):
        ...
        if cfg.model.view_embed_dim > 0:
            self.view_embed = nn.Parameter(torch.randn(1, cfg.n_views, cfg.model.view_embed_dim,1,1,1) * (1. / cfg.model.view_embed_dim) ** 0.5) # 1, n_views, C, H, W
    ...
    def forward(self, batch, with_fine=False, return_buffer=False):
        ...
        h, w = self.cfg.model.padding_size # [864, 1104]
        ...
        #### adjust input size to fit pretrained dino model ####
        # pad_h = h - H
        # pad_w = w - W
        # pad_h_half = pad_h // 2
        # pad_w_half = pad_w // 2
        # inps_img = batch['tar_rgb'][:,:n_views_sel].clone().detach().cpu().numpy()
        # pad_color = batch['bg_color'][:,:n_views_sel].clone().detach().cpu().numpy()
        # _inps_padded = torch.empty((B, n_views_sel, h, w, C))
        # for b in range(B):
        #     for v in range(n_views_sel):
        #         padding = [float(pad_color[b, v, 0] * 255), float(pad_color[b, v, 1] * 255), float(pad_color[b, v, 2] * 255)]
        #         padded_img = cv2.copyMakeBorder(inps_img[b, v] * 255., pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=padding)
        #         _inps_padded[b, v] = torch.from_numpy(np.array(padded_img).astype(np.float32) / 255.)
        # _inps_padded = _inps_padded.cuda().reshape(B*n_views_sel,h,w,C)
        # _inps_padded = torch.einsum('bhwc->bchw', _inps_padded)
        # pad_info = [pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half]
        ########################################################

        # image encoder
        # img_feats = torch.einsum('blc->bcl', self.img_encoder(_inps_padded))
        ...
        ################### unpad img_feats ####################
        # img_feats_unpad = img_feats[:, :, pad_info[2] : img_feats.shape[2] - pad_info[3]]
        # token_size = int(np.sqrt(H*W/img_feats_unpad.shape[-1]))
        # img_feats_unpad = img_feats_unpad.reshape(*img_feats_unpad.shape[:2],H//token_size,W//token_size)
        ########################################################
        
        feat_vol = self.build_feat_vol(_inps, img_feats, n_views_sel, batch) # B n_views_sel C D H W
        # feat_vol = self.build_feat_vol(_inps_padded, img_feats, n_views_sel, batch)
